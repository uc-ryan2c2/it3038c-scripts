...
def PasswdGenerator():
      ...
      # password generated
      Random_password = Label(Toplevel, text="Password")
      Random_password.grid(row=0)
      entry = Entry(Toplevel)
      entry.grid(row=0, column=1)
 
      # create label for length of password
      c_label = Label(Toplevel, text="Length")
      c_label.grid(row=1)
 
      # create Buttons Copy which will copy
            # password to clipboard and Generate
      # which will generate the password
      copy_button = Button(Toplevel, text="Copy", command=copy1)
      copy_button.grid(row=0, column=2)
      generate_button = Button(Toplevel, text="Generate", command=generate)
      generate_button.grid(row=0, column=3)
 
      # Radio Buttons for deciding the
      # strength of password
      # Default strength is Medium
      radio_middle = Radiobutton(Toplevel, text="Medium", variable=var, value=0)
      radio_middle.grid(row=1, column=2, sticky='E')
      radio_strong = Radiobutton(Toplevel, text="Strong", variable=var, value=3)
      radio_strong.grid(row=1, column=3, sticky='E')
      combo = Combobox(Toplevel, textvariable=var1)
...
#Main Buttons
GenPasswd = Button(frame, text="Generate Password", padx=40, pady=30, fg="white", bg="black", command=PasswdGenerator)
